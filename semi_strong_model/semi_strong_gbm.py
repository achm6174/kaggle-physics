...
Semi-strong model based on Gradient Boosting (xgboost) with hyperparameters optimization
...
for temp_max_depth in [4,5,6,7]:
    for temp_child_weight in [1,5,10,50,100]:
        for i in range(0,5):
            ...
            dtrain = xgb.DMatrix(scaler.fit_transform(np.array(train_table[i][features])), label = train_table[i]["signal"])
            ...
            # Prediction
            preds = gbm.predict(xgb.DMatrix(scaler.transform(np.array(train_eval_table[i][features]))))
